...
export default class Watcher{ // 用于Vue渲染虚拟DOM、实现watch选项/$watch方法、实现计算属性
    ...
    update(force){
        if(this.lazy){ // 对于计算属性的watcher，则暂时先不通知更新
            ...
            this.asyncRun(this); // 先将当前watcher放入队列中再批量更新
        }
    }
    run(){
    ...
    addDep(dep){
        ...
        if(!this.depIds.has(id)){ // 根据id判断该Dep实例是否已经被缓存过，没被缓存过才会进行下面这些操作。
            ...
            dep.addWatcher(this); // 将该watcher缓存Dep实例内部，之后属性更新时会通过Dep实例通知watcher更新——watcher调用run方法执行回调，从而实现组件视图更新/触发watch回调触发。
        }
    }
    depend(){ // 通过watcher的deps属性实现Dep.target对当前watcher(计算属性的watcher)的依赖。
